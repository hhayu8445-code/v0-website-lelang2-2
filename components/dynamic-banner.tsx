...
export async function DynamicBanner({ position }: { position: string }) {
  ...
  // Jika tidak ada banner dari database, tampilkan banner default
  if (!banners || banners.length === 0) {
    return (
      <div className="space-y-6">
        <div className="relative rounded-3xl overflow-hidden glass-card border-primary/20 shadow-2xl">
          ...
          <div className="absolute inset-0 z-20 flex flex-col justify-center px-4 sm:px-8 md:px-16">
            <div className="max-w-2xl space-y-3 sm:space-y-4">
              ...
              <p className="text-sm sm:text-lg md:text-xl text-white/90 drop-shadow-lg">
                Daftar sekarang dan verifikasi KYC untuk mendapatkan bonus saldo!
              </p>
              <Link href="/register">
