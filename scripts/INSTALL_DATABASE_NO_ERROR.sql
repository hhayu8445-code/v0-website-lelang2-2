-- ============================================
-- COMPLETE DATABASE SETUP - 100% FINAL
...
DROP VIEW IF EXISTS vehicle_analytics CASCADE;
...
DROP POLICY IF EXISTS "Users can create KYC" ON public.kyc_verifications;
CREATE POLICY "Users can create KYC" ON public.kyc_verifications FOR INSERT WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Admins can manage KYC" ON public.kyc_verifications;
...
DROP POLICY IF EXISTS "Public can view featured testimonials" ON public.testimonials;
...
DROP POLICY IF EXISTS "Admins can manage SEO" ON public.seo_metadata;
...
SELECT 
  u.id, u.full_name, u.email, u.kyc_status, u.wallet_balance, u.bonus_balance, u.auction_participation_count,
...
SELECT 
  v.id, v.brand, v.model, v.year, v.starting_price, v.current_bid, v.auction_status, v.view_count, v.bid_count,
...
FROM public.users GROUP BY DATE(created_at) ORDER BY date DESC;
